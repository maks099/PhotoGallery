...
class PreferencesRepository private constructor(private val dataStore: DataStore<Preferences>) {
    ...
    val isPolling : Flow<Boolean> = dataStore.data.map {
        it[PREF_IS_POOLING] ?: false
    }.distinctUntilChanged() as Flow<Boolean>

    suspend fun setPooling(isPooling: Boolean){
        dataStore.edit {
            it[PREF_IS_POOLING] = isPooling
        }
    }

    companion object{
        private val SEARCH_QUERY_KEY = stringPreferencesKey("searchQUERY")
        private val PREF_LAST_RESULT_ID = stringPreferencesKey("lastResultId")
        private val PREF_IS_POOLING = booleanPreferencesKey("isPooling")
